...
import moves
import cube
# import kociemba  #Kociemba uses a two step algorithm to solve a rubiks cube

#displaying the cuve faces 
def show(side):
    surr = {
        0:"2435",
        1:"2534",
    ...
    #Placing the centers around the main 3x3
...
#Solving using 2 step algorithm
def solve(opt):
...
#Change the colour of the clicked cube
def edit_colour(i):
    cur = buts[4].cget('bg')
    face = {
        cols[0]:cube.green,
...
def main(root):
    ...
    #DEFINING FRAMES
    title = LabelFrame(root,padx = 400)
    dispcube = LabelFrame(root,padx = 10,pady = 20,text = "Cube")
    modeSel = LabelFrame(root,padx = 60, pady = 10, text = "Mode")
    colourSel = LabelFrame(root,padx = 35,pady = 10,text = "Color")
    scramble = LabelFrame(root,text = "Scramble(Green is Front; White on Top)")
    options = LabelFrame(root,text = "Options",padx = 80,pady = 10)
    output = LabelFrame(root,padx = 2, pady = 2,text = "Solution(Green is Front; White on Top)")
    ...
    #TITLE
    Label(title,text = "Solver",pady = 3).grid(row = 0, column = 0)

    #MODE SELECTOR
    mode = IntVar()
    Radiobutton(modeSel, text = "Browse", variable = mode, value = 0,command = lambda:browse(mode)).pack(anchor = W)
    Radiobutton(modeSel, text = "Edit", variable = mode, value = 1,command = edit).pack(anchor = W)
